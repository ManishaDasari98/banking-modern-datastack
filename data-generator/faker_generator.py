    ...
    for _ in range(NUM_TRANSACTIONS):
        ...
        if txn_type == "TRANSFER" and len(accounts) > 1:
            ...
            if account_balances[account_id] >= amount:
                ...
                account_balances[related_account] += amount
            else:
                # Skip transfer if insufficient funds
                continue
        ...
        cur.execute(
            ...
            (account_balances[account_id], account_id),
        )
        if related_account:
            cur.execute(
                "UPDATE accounts SET balance = %s WHERE id = %s",
                (account_balances[related_account], related_account),
            )
